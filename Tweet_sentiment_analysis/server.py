...
def add_color(tweets):
    ...
    to an index from 0..99. That index gives you the color increment
...
def tweets(name):
    ...
    if median < -0.33:
        ...
        category = "Strong"
    record = add_color(tweets)
    return render_template("tweets.html", median = median, category = category, record = record)
...
def following(name):
    ...
    """
    content = fetch_following(api,name)
    newlist = sorted(content, key=lambda x: x['followers'], reverse=True)
    return render_template("following.html", name = name, content = newlist)
